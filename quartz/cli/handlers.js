  ...
  if (argv.serve) {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      const serveFile = (filePath) => {
        ...
        console.log(styleText("green", "[200]") + styleText("gray", ` ${argv.baseDir}${req.url}`))
      }
      ...
      if (fp.endsWith("/")) {
        ...
        const outputIndexFp = path.posix.join(argv.output, indexFp)
        if (fs.existsSync(outputIndexFp)) {
          return serveFile(outputIndexFp)
        }
